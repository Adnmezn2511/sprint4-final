...
from . import logger_setup, constants
from .BipartiteAll import BipartiteAll
from .Bipartite import Bipartite
from .MultiplexAll import MultiplexAll
from .Multiplex import Multiplex
from .MultiplexLayer import MultiplexLayer
from .ParameterEta import ParameterEta
from .ParameterLambda import ParameterLambda
...
class ConfigParser:
    ...
    def parse_old(self):
        ...
        self.multiplexall_obj = self.__parse_multiplex_old()
    ...
    def __parse_multiplex(self):
        ...
        for multiplex_idx, multiplex_key in enumerate(config_multiplex_dic):
            ...
            for layer_idx, layer_key in enumerate(layer_key_tuple):
                ...
                multiplex_node_list = sorted([*set(multiplex_node_list + [*layer_obj.networkx_graph.nodes])])
            ...
            for layer_idx, layer_obj in enumerate(layer_obj_list):
                layer_missing_nodes = sorted(set(multiplex_node_list) - set(list(layer_obj.networkx_graph.nodes)))
                layer_obj.networkx_graph.add_nodes_from(layer_missing_nodes)
    ...
    def __parse_multiplex_old(self):
        """
        Reads multiplex field and create MultiplexAll object
        """
        ...
        for multiplex_idx, multiplex_key in enumerate(config_multiplex_dic):
            ...
            for layer_idx, layer_key in enumerate(layer_key_tuple):
                ...
                layer_obj = MultiplexLayer(key=layer_key, abspath=layer_abs_path,
                                           graph_type=graph_type_lst[layer_idx], multiplex=multiplex_key, tau=tau_lst[layer_idx], self_loops=self.self_loops)
                multiplex_node_list = sorted([*set(multiplex_node_list + [*layer_obj.networkx_graph_old.nodes])])
                layer_obj_list.append(layer_obj)

            # Append missing nodes from other layers in one multiplex to each layer
            for layer_idx, layer_obj in enumerate(layer_obj_list):
                layer_missing_nodes = sorted(set(multiplex_node_list) - set(list(layer_obj.networkx_graph_old.nodes)))
                layer_obj.networkx_graph_old.add_nodes_from(layer_missing_nodes)
                layer_obj_list[layer_idx] = layer_obj  # update
