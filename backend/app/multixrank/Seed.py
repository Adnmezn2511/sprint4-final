...
class Seed:
    ...
    def seed_list(self) -> list:
        ...
        if not self._seed_list:
            ...
            if len(seed_list) <= 0:  # no seeds
                logger_setup.logger.error("No seed nodes were provided.")
                raise ValueError("No seed nodes were provided.")
            ...
            if len(seeds_not_in_multiplex_set) > 0:  # seeds not in multiplex
                logger_setup.logger.warning(
                    "These seed nodes were not found in the network and will be ignored: {}".format(
                        seeds_not_in_multiplex_set))
                self._missing_seeds = list(seeds_not_in_multiplex_set)

            valid_seed_list = sorted(set(seed_list) - seeds_not_in_multiplex_set)
            if len(valid_seed_list) <= 0:
                logger_setup.logger.error("No valid seed nodes were found in the network.")
                raise ValueError("No valid seed nodes were found in the network.")

            self._seed_list = valid_seed_list
    ...
    @property
    def missing_seeds(self) -> list:
        """Returns seed labels that were not found in the multiplex."""

        if not self._seed_list:
            _ = self.seed_list

        return self._missing_seeds
